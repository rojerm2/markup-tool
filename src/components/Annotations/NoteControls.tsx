...
export default function NoteControls({notes,selected,pointer,onSelect,editing,onEdit,onClose,onPointer,placing,history,dispatch,disabled,revision}: {
  ...
  const [draft,setDraft]=useState<TextNote|null>(null),[error,setError]=useState<string|null>(null),[generation,setGeneration]=useState(0);
  ...
  useEffect(()=>{const cancel=(e?:Event)=>{if(e?.type==='scroll'&&e.target instanceof Element&&e.target.closest('.note-properties'))return;setDraft(null);onClose();};const key=(e:KeyboardEvent)=>{if(e.key==='Escape')cancel();};window.addEventListener('keydown',key);window.addEventListener('blur',cancel);window.addEventListener('scroll',cancel,true);return()=>{window.removeEventListener('keydown',key);window.removeEventListener('blur',cancel);window.removeEventListener('scroll',cancel,true);};},[onClose]);
  ...
  return <div className="note-controls">
    ...
    {open&&(n||draft)&&<div className="note-properties" role="dialog" aria-label="Note and arrow properties">
      ...
      {draft?<>
        ...
        <label>Text size<select aria-label="Text size" value={draft.fontSize} onChange={e=>setDraft({...draft,fontSize:Number(e.target.value)})}>{[10,12,16,20].map(v=><option key={v} value={v}>{v} pt</option>)}</select></label>
        ...
        <button disabled={disabled} onClick={()=>{if(!draft.text.trim()&&!editing?.text){setDraft(null);onClose();return;}const err=noteTextError(draft.text);if(err){setError(err);return;}if(!Number.isFinite(draft.width)||draft.width<40||draft.width>2000){setError('Use a width from 40 to 2000 PDF units.');return;}const value=fitNote(draft);if(!validNote(value)){setError('Enter nonempty text that fits within the supported box size.');return;}dispatch({type:'put-note',note:value,before:editing?.text?editing:undefined},generation);setDraft(null);onClose();onSelect(value.id);}}>Apply text</button>
        ...
        {style&&<><label>Color<select aria-label="Arrow color" value={style.color} onChange={e=>property('color',e.target.value)}>{!COLORS.some(c=>c.value===style.color)&&<option value={style.color}>Saved color</option>}{COLORS.map(c=><option key={c.value} value={c.value}>{c.name}</option>)}</select></label>
          <label>Shaft width<select aria-label="Arrow shaft width" value={style.width} onChange={e=>property('width',Number(e.target.value))}>{![1,2,4].includes(style.width)&&<option value={style.width}>{style.width}</option>}{[1,2,4].map(v=><option key={v} value={v}>{v} pt</option>)}</select></label>
          <label>Arrowhead<select aria-label="Arrowhead size" value={style.head} onChange={e=>property('head',Number(e.target.value))}>{![0,6,10,16].includes(style.head)&&<option value={style.head}>{style.head}</option>}{[0,6,10,16].map(v=><option key={v} value={v}>{v?`${v} pt`:'No head'}</option>)}</select></label></>}
