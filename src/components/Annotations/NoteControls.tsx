...
export default function NoteControls({notes,selected,pointer,onSelect,editing,onEdit,onClose,onPointer,placing,history,dispatch,disabled,revision}: {
  ...
  useEffect(()=>{const cancel=(e?:Event)=>{if(e?.type==='scroll'&&e.target instanceof Element&&e.target.closest('.note-properties, .workspace-panel'))return;setDraft(null);onClose();};const key=(e:KeyboardEvent)=>{if(e.key==='Escape')cancel();};document.addEventListener('visibilitychange',cancel);window.addEventListener('keydown',key);window.addEventListener('blur',cancel);window.addEventListener('scroll',cancel,true);return()=>{document.removeEventListener('visibilitychange',cancel);window.removeEventListener('keydown',key);window.removeEventListener('blur',cancel);window.removeEventListener('scroll',cancel,true);};},[onClose]);
