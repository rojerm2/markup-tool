...
export default function ShapeControls({shapes,selected,onSelect,value,onChange,onDelete,line=false}: {
  shapes:Shape[];selected:string|null;onSelect:(id:string|null)=>void;value:Pick<Shape,'color'|'width'|'fill'>;onChange:(style:Pick<Shape,'color'|'width'|'fill'>)=>void;onDelete?:()=>void;line?:boolean;
}) {
  ...
  return <div className="shape-controls" role="group" aria-label="Shape properties">
    ...
    <label>Outline <select aria-label="Shape outline" value={value.color} onChange={e=>onChange({...value,color:e.target.value})}>
